export function renderNavbar() {
  return `
    <header id="header" class="h-16 border-b border-slate-800 bg-slate-900/50 backdrop-blur flex items-center justify-between px-4 sm:px-6 sticky top-0 z-30">
      
      <!-- LADO IZQUIERDO: Menú Hamburguesa, Logo Móvil, Ubicación y Reloj -->
      <div class="flex items-center gap-3 sm:gap-4">
        <!-- BOTÓN QUE ABRE EL MENU EN MÓVILES -->
        <button id="toggle-sidebar-btn" class="p-2 text-slate-400 hover:text-white lg:hidden focus:outline-none rounded-lg hover:bg-slate-800 transition cursor-pointer">
      ...
      <div class="flex items-center gap-3">
        <button id="btn-voice-command" title="Dictar comando por voz" class="p-2 border border-slate-700 rounded-lg text-slate-300 hover:text-sky-400 hover:border-sky-500/50 transition-all flex items-center gap-2 text-xs font-medium bg-slate-800/40 cursor-pointer">
